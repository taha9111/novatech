...
window.updateProfilePhoto = async function () {
  ...
  window.getFreeCourse = async function () {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/user/enroll/6868843691f59b8c38d3896c`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  window.buyCourse = async function (courseId) {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/courses/buy/${courseId}`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${token}`,
  ...
  async function fetchProfile() {
    ...
    try {
      ...
      if (profileInfo) {
          ...
          document.getElementById("profileImg").src = `http://localhost:5000/uploads/${user.photo}`;
  ...
  async function fetchCourses() {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/user/my-courses", {
        headers: { Authorization: `Bearer ${token}` },
      });
